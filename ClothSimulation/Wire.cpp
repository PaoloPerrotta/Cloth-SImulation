...
Wire::Wire(int resolution, int size){
    ...
    //Generic formula for simplified model ((3*res)*res)+2*res)
...
//add a single particle to the wire
...
//make particles visible, rendering with opengl vertex size
...
//updates cloth physics
void Wire::update(int sphereRadius, float gravityForce, Vector gravityDirection, float ks, float kd, float particleRadius, float *objPos, int floorY){
    //apply gravity on particles
    for(int i = 0; i < resolution; i++){
        for(int j = 0; j < resolution; j++){
            particles[i][j].dim = particleRadius;
    ...
    for(int i = 0; i < resolution; i++){
        for(int j = 0; j < resolution; j++){
            particles[i][j].update(sphereRadius, objPos, floorY);
        }
    }
}

void Wire::generateSprings(){
    /*The cloth is seen as consisting of triangles for four particles as follows:
    ...
    for(int i = 0; i < resolution; i++){
        for(int j = 0; j < resolution; j++){
            ...
            if(i == resolution-1 && j < resolution-1){
                ...
                //We are at the top of the cloth, we will just create 2 springs here
            ...
            //we are at the bottom of the cloth
                springs[springIndex] = new Spring(&particles[i][j], &particles[i+1][j-1], diagonalDelta);
                springIndex++;
